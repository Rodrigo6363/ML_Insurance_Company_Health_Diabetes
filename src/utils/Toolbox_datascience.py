...
def plot_categorical_distribution_by_target(data, cat_columns=None, target_column=None, palette='Set2', cols_per_row=3):
    """
    Genera gráficos de barras para visualizar la relación entre variables categóricas y el target.

    Parámetros:
    - data: DataFrame de pandas con los datos.
    - cat_columns: Lista de nombres de columnas categóricas. Si None, se seleccionan automáticamente.
    - target_column: Nombre de la variable categórica objetivo.
    - palette: Paleta de colores de Seaborn.
    - cols_per_row: Número de gráficos por fila en la cuadrícula.
    """
    ...
    if target_column is None:
        raise ValueError("Debes proporcionar una variable target categórica.")

    # Excluir la variable target si está en las categóricas
    cat_columns = [col for col in cat_columns if col != target_column]

    # Calcular el número de filas necesarias en la cuadrícula
    num_plots = len(cat_columns)
    n_rows = math.ceil(num_plots / cols_per_row)

    # Crear la figura con subgráficos
    fig, axes = plt.subplots(n_rows, cols_per_row, figsize=(cols_per_row * 5, n_rows * 5))
    axes = axes.flatten()  # Asegurar que los ejes sean un array plano

    # Generar gráficos de barras para cada variable categórica
    for i, col in enumerate(cat_columns):
        sns.countplot(data=data, x=col, hue=target_column, ax=axes[i], palette=palette)
        axes[i].set_title(f"Distribución de {col} por {target_column}")
        axes[i].tick_params(axis='x', rotation=45)  # Rotar etiquetas si son largas

    # Eliminar gráficos vacíos si hay más subplots de los necesarios
...
def evaluate_classification_models(X_train_scaled, y_train, scoring="accuracy"):
    """
    Trains and evaluates multiple classification models using cross-validation.

    Parameters:
        X_train_scaled: Scaled training data.
        y_train: Training labels.
        scoring: Evaluation metric for cross_val_score (default is "accuracy").

    Returns:
        A DataFrame with the evaluation results of all models.
    """
    model_names = [
    ...
    for name, model in zip(model_names, model_set):
        print(f"Evaluating model: {name} using metric {scoring}...")
        try:
            scores = cross_val_score(model, X_train_scaled, y_train, cv=3, scoring=scoring, n_jobs=-1)
            cv_metrics[name] = scores
            scores_list.append(np.mean(scores))
        except Exception as e:
            print(f"Error with model {name}: {e}")
            cv_metrics[name] = None
            scores_list.append(-np.inf)

    winner = model_names[np.argmax(scores_list)]
    print(f"\nThe best model is: {winner} with a mean {scoring} of {np.max(scores_list):.4f}\n")

    results_df = pd.DataFrame({
        "Model": model_names,
        f"Mean {scoring.capitalize()}": scores_list
    }).sort_values(by=f"Mean {scoring.capitalize()}", ascending=False)

    print(results_df)

    plt.figure(figsize=(12, 6))
    sns.barplot(x=f"Mean {scoring.capitalize()}", y="Model", data=results_df, palette="viridis")
    plt.title(f"Model Comparison - Mean {scoring.capitalize()}", fontsize=16)
    plt.xlabel(f"Mean {scoring.capitalize()}", fontsize=12)
